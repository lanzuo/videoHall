...
- (id)initWithFrame:(CGRect)frame delegate:(id<SGFocusImageFrameDelegate>)delegate ImageItems:(NSMutableArray *)imageItems
{
    self = [super initWithFrame:frame];
    if (self) {
...
//        imageView.image = item.image;
        ...
        lbltitle.font = [UIFont fontWithName:@"FZLTCXHJW--GB1-0" size:18];
...
- (void)singleTapGestureRecognizer:(UIGestureRecognizer *)gestureRecognizer
{
    ...
    if (page > -1 && page < imageItems.count) {
        ...
        if ([self.delegate respondsToSelector:@selector(foucusImageFrame:didSelectItemIndex:)]) {
            [self.delegate foucusImageFrame:self didSelectItemIndex:item.tag];
        }
    }
    //objc_setAssociatedObject(self, (const void *)SG_FOCUS_ITEM_ASS_KEY, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
-(void)clickPageImage:(UIButton *)sender{
    ...
    if (self.delegate) {
        [self.delegate foucusImageFrame:self didSelectItemIndex:btn.tag];
    }
}
